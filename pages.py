...
from config import label, is_infrastructure, is_monitored, is_pauseable, effective_type, VERSION
...
def send_test_ntfy(config):
    ...
    try:
        dash_url    = config.get("dashboard_url", "http://192.168.8.1:8081")
...
def build_social(config, saved=False, error=""):
    current_profile = config.get("social_profile", "moderate")
    ...
    for pid, plabel_str, pdesc, picon, pcolor, pbg, pborder in [
        ("open",     "Open",     "All social media allowed.",                                          "&#x1F513;", "#16A34A", "#F0FDF4", "#86EFAC"),
        ("moderate", "Moderate", "All social media allowed. Adult content blocked by AdGuard.",        "&#x1F6E1;", "#D97706", "#FFFBF0", "#FEF3C7"),
        ("teen",     "Teen",     "YouTube and Discord only. All other platforms blocked.",             "&#x1F393;", "#6366f1", "#F0F0FF", "#E0E0FF"),
        ("strict",   "Strict",   "All social media blocked.",                                          "&#x1F512;", "#DC6B5F", "#FFF7F7", "#FCA5A5"),
        ("custom",   "Custom",   "Choose exactly which platforms to allow.",                           "&#x2699;",  "#475569", "#F8FAFC", "#E2E8F0"),
    ]:
    ...
    for pid2, plabel2, picon2 in platforms:
        chk         = "checked" if pid2 in custom.get("platforms", list(PLATFORM_DOMAINS.keys())) else ""
    ...
    custom_section = (
        ...
        '<div style="font-weight:700;color:#475569;margin-bottom:10px">&#x2699; Custom Profile — Choose Allowed Platforms</div>'
    ...
    return (
        ...
        + '<div class="section">'
        + info_banner
        + profile_cards + custom_section + extra_protection
        + '</div></div></body></html>'
    )
...
def build_devices_page(config, saved=False, redetect=False, autoname=False, sort="name", flt=""):
    ...
    for d in all_devices:
        ...
        # Resolve display name: if client_name is a bare IP, look up hostname
        if _is_ip(name):
            raw_host    = ip_hostnames.get(name, "")
            display     = pretty_hostname(raw_host) if raw_host else name
...
def build_blocked_services_page(all_svcs, blocked_ids, ss_on, config, saved_msg=""):
    ...
    if not all_svcs:
        ...
        body = (
            ...
            f'Tick individual sites, or use All / None. A new block takes a few seconds to apply.</div>'
            f'</div>'
            f'{pack_blocks}'
    ...
    custom_section = (
        ...
        f'A new block takes a few seconds to apply. To edit one, remove it and add the corrected version.</div>'
...
def build_admin(config, saved=False, cleared=False, cleared_all=False,
    ...
    return (
        ...
        + '      res.textContent="✅ You\'re up to date! (v"+d.latest_version+")";'
        + '    }'
        ...
        + f'<div class="form-card" id="doh-setting" style="scroll-margin-top:70px"><div class="form-label">Encrypted-DNS Bypass Blocking</div>'
        + f'<div style="color:#94a3b8;font-size:0.78em;margin-bottom:10px">When enabled, devices can\'t use encrypted DNS (DoH/DoT) to sneak around your filters &mdash; all DNS is forced through Lantern Watch. Closes a loophole tech-savvy kids use, but can interrupt some browsers, apps, or smart TVs that insist on their own DNS. Optional &mdash; leave off for typical family use, turn on if you suspect filter bypass.</div>'
        + f'<label class="toggle-row"><input type="checkbox" name="doh_blocking" {doh_on}>'
        + f'<span>Block encrypted-DNS bypass (DoH/DoT)</span></label></div>'
...
def build_notifications(config, cleared=False, saved=False,
    ...
    notif_form = (
        ...
        # ntfy
        + '<div class="form-card">'
        + f'<label style="display:flex;align-items:center;gap:10px;margin-bottom:12px;cursor:pointer"><input type="checkbox" name="ntfy_enabled" {"checked" if config.get("ntfy_enabled", True) else ""} style="width:18px;height:18px;accent-color:#e8a000"><span style="font-weight:700;color:var(--ink)">ntfy push notifications</span></label>'
        ...
        + f'<label style="display:flex;align-items:center;gap:10px;margin-bottom:12px;cursor:pointer"><input type="checkbox" name="telegram_enabled" {"checked" if _tg.get("enabled", True) else ""} style="width:18px;height:18px;accent-color:#e8a000"><span style="font-weight:700;color:var(--ink)">Telegram notifications enabled</span></label>'
        ...
        + f'<label style="display:flex;align-items:center;gap:10px;margin-bottom:12px;cursor:pointer"><input type="checkbox" name="email_enabled" {"checked" if _em.get("enabled", True) else ""} style="width:18px;height:18px;accent-color:#e8a000"><span style="font-weight:700;color:var(--ink)">Email notifications enabled</span></label>'
        ...
        + f'<label class="check-row"><input type="checkbox" name="alert_vpn" {_vpn}><span>Possible VPN detected</span></label></div>'
