...
[ -z "$RAW_MODEL" ] && die "Could not detect router model. Use --force to proceed with safe defaults."
...
case "$RAW_MODEL" in
    ...
    *)
        if [ "$FORCE" = "yes" ]; then
            log "WARNING: Unknown model '$RAW_MODEL' — proceeding with conservative defaults (--force)"
            MODEL_NAME="unknown"
            HAS_WIFI="no"
            DISABLE_WIFI="no"
            LAN_IFACE="br-lan"
            ...
            echo ""
            echo "ERROR: Unknown router model: $RAW_MODEL"
            echo ""
            echo "Supported models:"
            echo "  GL-MT6000  (Flint 2)"
            echo "  GL-MT3000  (Beryl AX)"
            echo "  GL-MT2500  (Brume 2)"
            echo "  GL-MT5000  (Brume 3)"
            echo ""
            echo "To proceed with conservative defaults: sh install.sh --force"
            echo ""
            trap - EXIT
            exit 1
        fi
...
try:
    ...
    with open("$CONFIG") as f:
        config = json.load(f)
    NEW_UP = ["https://cloudflare-dns.com/dns-query", "https://dns.quad9.net/dns-query"]
    BOOT   = ["1.1.1.1", "9.9.9.10", "149.112.112.10"]
    ...
    if res and all(str(v).upper().startswith("OK") for v in res.values()):
        ...
        print("upstreams=encrypted (Cloudflare + Quad9 DoH)")
    else:
...
awk '
  ...
  section=="filtering" && /^[[:space:]]*safebrowsing_enabled:[[:space:]]*false/ {
    sub(/safebrowsing_enabled:.*/, "safebrowsing_enabled: true")
  }
  section=="filtering" && /^[[:space:]]*parental_enabled:[[:space:]]*false/ {
    sub(/parental_enabled:.*/, "parental_enabled: true")
  }
  { print }
...
sleep 5
log "  AdGuard global safebrowsing + parental enabled via config.yaml (per-client overrides preserved)."
...
echo ""
echo "======================================================================"
echo "  Lantern Watch: A light to your home network."
echo "======================================================================"
echo ""
...
echo ""
echo "  👉 Access your dashboard to finish setup:"
echo "        http://$LAN_IP:8081   (or http://lanternwatch.lan:8081)"
echo ""
...
echo ""
echo "  Privacy: this install is counted once, anonymously (a random ID +"
echo "           router model + version only). Nothing else is ever sent"
echo "           unless you turn on Settings -> Share anonymous usage stats."
echo ""
echo "============================================="
echo ""
