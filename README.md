...
- **Device types** — label devices as Person/Child, Parent/Admin, Work Device, Infrastructure, or Smart Device; Work Device bypasses all AdGuard filtering automatically
...
- **Beryl 7 (GL-MT3600BE)** — MediaTek quad-core @ **2.0 GHz**, **512 MB DDR4 RAM, 512 MB** NAND storage, 2x 2.5G ports, Wi-Fi 7. Pocket-sized and travel-friendly with fast modern Wi-Fi — ideal for apartments, RVs, and second locations. With 512 MB RAM, keep filter lists moderate and expect shorter log retention than the Flint 2.
...
| Teen | Blocks TikTok, Twitter/X, Snapchat, Reddit, Twitch |
| Strict | All 10 platforms blocked |
| Custom | Choose exactly which platforms are allowed |

Changes apply instantly to every device on the network.

---

## Device types

Go to **Devices** in the top nav to label every device and set its type.

| Type | Behaviour |
|---|---|
| 👤 Person / Child | Included in Pause All Person/Child |
| 🛡️ Parent / Admin | Never paused; full filtering applies |
| 💼 Work Device | Never paused; **all AdGuard filtering disabled** for this device |
| 🖥️ Infrastructure | Never paused (NAS, printer, router) |
| 📡 Smart Device | Never paused (cameras, TV, doorbell, thermostat) |

**Work Device** is designed for laptops or phones that need unobstructed access to corporate VPNs, video calls, and work tools. When you set a device to Work Device, Lantern Watch creates a per-client entry in AdGuard Home that disables all blocking rules for that device. Changing it back to any other type removes the entry automatically.
...
3. SSH into the router and pull:
```bash
ssh root@192.168.8.1 "cd /root/lantern-watch && git pull origin main && /etc/init.d/lanternwatch restart"
```

The dashboard is then live at **http://192.168.8.1:8081**.
...
### 0.9.0-beta — architecture-review hardening (2026-06-19)

A correctness and resilience pass following a review of the AdGuard Home and GL.iNet integration:

- **Query log no longer loses entries on busy networks.** The collector now pages backward through the AdGuard query log instead of only reading the newest 500 entries per poll, so adult-content alerts, screen time, and block-rate detection stay accurate under load.
- **Concurrent rule writes are serialized.** Social blocking, DoH blocking, and the domain allowlist share AdGuard's custom-rules list; writes are now lock-protected so simultaneous changes can't clobber each other.
- **Safe Browsing / Parental toggles are verified, not assumed.** The setup wizard confirms these settings against AdGuard's status API and surfaces a clear warning if they didn't take (some GL.iNet builds reject the API call), instead of reporting false success.
- **Survives "Keep Settings" firmware upgrades.** The installer registers the app, config, and database in `/etc/sysupgrade.conf`.
- **Installer no longer overrides per-client filtering.** The Safe Browsing / Parental config edit is now scoped to AdGuard's global settings only, leaving per-device overrides intact.
- **Docs corrected.** Social blocking is AdGuard custom filter rules via the `/filtering/set_rules` API (async, no DNS restart, no network interruption) — not dnsmasq.
...
If your speeds are significantly lower than these, the most common cause is too many AdGuard filter lists loaded at once. The wizard installs a balanced set (~350K rules). Avoid adding very large lists — anything over 500K rules is heavy for home router hardware and can cause AdGuard to crash and restart under memory pressure.
