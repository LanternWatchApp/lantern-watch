...
SOCIAL_PROFILES = {
    ...
    "teen":     ["youtube", "discord", "instagram", "facebook", "pinterest"],          # only these allowed
...
RECOMMENDED_LISTS = [
    {
        ...
        "description": "AdGuard's primary list — ads, trackers, and malicious sites",
...
def set_safesearch_enabled(config, enabled):
    """Enable or disable safe search for image/search engines.

    YouTube is deliberately left OFF: turning on YouTube safe search forces
    YouTube "Restricted Mode", which YouTube itself uses to hide ALL comments
    (viewing and posting). Image safety comes from Google/Bing/DuckDuckGo/etc.;
    YouTube's own content restriction is better done per-account (Family Link /
    supervised YouTube accounts), so families keep normal YouTube + comments."""
    payload = json.dumps({
...
def _ag_post(config, path, payload):
    try:
        data = json.dumps(payload).encode()
        with urllib.request.urlopen(_ag_request(config, "/control" + path, data), timeout=5) as r:
            return True
    except Exception as e:
