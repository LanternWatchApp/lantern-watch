...
def _hardware_install_id():
    """Derive a stable UUID from the router's MAC address using a one-way hash.
    Same physical router always produces the same ID across reinstalls and resets.
    Falls back to a random UUID if the MAC is unreadable."""
    import hashlib, uuid
    try:
        for iface in ("eth0", "eth1", "br-lan"):
            try:
                mac = open(f"/sys/class/net/{iface}/address").read().strip()
                if mac and mac != "00:00:00:00:00:00":
                    h = hashlib.sha256(b"lanternwatch:" + mac.encode()).digest()
...
def get_or_create_install_id():
    """Return a stable anonymous ID for this installation.
    Derived from router hardware so reinstalls don't generate duplicate IDs."""
    ...
    if row:
        ...
        install_id = _hardware_install_id()
