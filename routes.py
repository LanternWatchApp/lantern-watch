...
from config import load_config, save_config, label, is_first_run, is_pauseable, VERSION, UPDATE_CHECK_URL, UPDATE_RELEASES_URL, is_newer_version
import recovery
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if _via_port80 and ("192.168.8.1" in host or not host):
            ...
            self._redirect("https://192.168.8.1")
            return
        ...
        self_hosts = {"192.168.8.1", "127.0.0.1", "localhost"}
        ...
        if host and host_name not in self_hosts:
            if config.get("captive_portal"):
                ...
                if not is_portal_acked(self.client_address[0], config):
                    ...
                    self._redirect(f"http://192.168.8.1:8081/portal?dest={_dest}")
                    return
            self._redirect("http://192.168.8.1:8081/blocked")
            return
        ...
        try:
            if parsed.path == "/setup/password":
                ...
                all_ids = [lst["id"] for lst in RECOMMENDED_LISTS]
    ...
    def do_POST(self):
        ...
        try:
            ...
            if parsed.path == "/portal/ack":
                ...
                self._redirect(dest or "http://192.168.8.1:8081/")
                return
